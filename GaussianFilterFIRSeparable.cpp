...
static inline void pv(Mat& src, Mat& dest, int top, int bottom, int type)
{
	int max_core = omp_get_num_procs();
	/*
	float* s = src.ptr<float>(0);
	float* d = dest.ptr<float>(top);
	memcpy(d, s, sizeof(float)*src.cols*(src.rows));
	*/
#pragma omp parallel for schedule(dynamic)
	for (int n = 0; n < max_core; n++)
	{
		...
		float* d = dest.ptr<float>(start + top);
		memcpy(d, s, sizeof(float)*src.cols*(end - start));
	}
...
#pragma omp for  nowait
		for (int j = 0; j < top; j++)
		{
			...
			memcpy(d, s, sizeof(float)*(src.cols));
		}
#pragma omp for nowait
...
void parallelCopyMakeBorder(Mat& src, Mat& dest, int top, int bottom, int left, int right, int type)
{
...
void GaussianFilterSeparableFIR::setTileDiv(const int tileDivX, const int tileDivY)
{
	...
	const int max_core = omp_get_num_procs();//max_core or 1
...
GaussianFilterSeparableFIR::GaussianFilterSeparableFIR()
{
	...
	const int max_core = omp_get_num_procs();
...
void GaussianFilterSeparableFIR::createGaussianTable(const int r, const float sigma)
{
	const int ksize = 2 * r + 1;
	if (gauss != NULL) _mm_free(gauss);
	gauss = (float*)_mm_malloc(sizeof(float)*ksize, 32);
...
void GaussianFilterSeparableFIR::filter(Mat& src, Mat& dest, const int r, const float sigma, int method, int border, int vectorization, bool useAllocBuffer)
{
	createGaussianTable(r, sigma);
	switch (method)
	{
	case FIR2D_Border:
		...
		for (int j = 0; j < src.rows; j++)
		{
			...
			for (int i = 0; i < r; i++)
			{
				...
				for (int k = 0; k < ksize; k++)
				{
					...
					v += (idx >= 0) ? gauss[k] * s[idx] : gauss[k] * constVal;
				}
				d[i] = v;
			}
		...
		for (int i = 0; i < src.cols; i++)
		{
			...
			for (int j = 0; j < r; j++)
			{
				...
				for (int k = 0; k < ksize; k++)
				{
					...
					v += (idx >= 0) ? gauss[k] * si[idx] : gauss[k] * constVal;
				}
				d[i] = v;
			}
		...
		for (int n = 0; n < max_core; n++)
		{
			...
			for (int i = start; i < end; i++)
			{
				...
				for (int j = 0; j < r; j += 8)
				{
					...
					d[wstep0] = mv.m256_f32[0];
					d[wstep1] = mv.m256_f32[1];
					d[wstep2] = mv.m256_f32[2];
					d[wstep3] = mv.m256_f32[3];
					d[wstep4] = mv.m256_f32[4];
					d[wstep5] = mv.m256_f32[5];
					d[wstep6] = mv.m256_f32[6];
					d[wstep7] = mv.m256_f32[7];
					d += wstep8;
			}
			...
			if (j < r)
			{
				for (int i = 0; i < src.cols; i++)
				{
					...
					for (int k = 0; k < ksize; k++)
					{
						int idx = j + k - r;
						idx = border_s(idx);
						v += (idx >= 0) ? gauss[k] * s[i + idx*wstep] : gauss[k] * constVal;
					}
					d[i] = v;
				}
			}
...
#pragma omp parallel for
		for (int j = 0; j < src.rows; j++)
		{
			...
			for (int i = 0; i < R; i += 8)
			{
				...
				d[bstep0] = mv.m256_f32[0];
				d[bstep1] = mv.m256_f32[1];
				d[bstep2] = mv.m256_f32[2];
				d[bstep3] = mv.m256_f32[3];
				d[bstep4] = mv.m256_f32[4];
				d[bstep5] = mv.m256_f32[5];
				d[bstep6] = mv.m256_f32[6];
				d[bstep7] = mv.m256_f32[7];
				d += bstep8;
		}
			for (int i = R; i < src.cols - R; i += 8)
			{
		...
		for (int n = 0; n < max_core; n++)
		{
			...
			for (int i = start; i < end; i++)
			{
				...
				for (int j = 0; j < src.rows; j += 8)
				{
					...
					d[dstep0] = mv.m256_f32[0];
					d[dstep1] = mv.m256_f32[1];
					d[dstep2] = mv.m256_f32[2];
					d[dstep3] = mv.m256_f32[3];
					d[dstep4] = mv.m256_f32[4];
					d[dstep5] = mv.m256_f32[5];
					d[dstep6] = mv.m256_f32[6];
					d[dstep7] = mv.m256_f32[7];
					d += dstep8;
				}
				s += bstep;
			}
...
void GaussianFilterSeparableFIR::filterVHImageBV(Mat& src, Mat& dest, const int r, const float sigma, int border, int opt, bool useAllocBuffer)
{
	...
	if (useParallelBorder)	parallelCopyMakeBorder(src, bufferImageBorder, r, r, 0, 0, border);
		...
		for (int n = 0; n < max_core; n++)
		{
			...
			for (int i = start; i < end; i++)
			{
				...
				for (int j = 0; j < r; j += 8)
				{
					...
					d[i + 0 * wstep] = mv.m256_f32[0];
					d[i + 1 * wstep] = mv.m256_f32[1];
					d[i + 2 * wstep] = mv.m256_f32[2];
					d[i + 3 * wstep] = mv.m256_f32[3];
					d[i + 4 * wstep] = mv.m256_f32[4];
					d[i + 5 * wstep] = mv.m256_f32[5];
					d[i + 6 * wstep] = mv.m256_f32[6];
					d[i + 7 * wstep] = mv.m256_f32[7];
	}
				for (int j = R; j < simdend; j += 8)
				{
					...
					d[i + wstep0] = mv.m256_f32[0];
					d[i + wstep1] = mv.m256_f32[1];
					d[i + wstep2] = mv.m256_f32[2];
					d[i + wstep3] = mv.m256_f32[3];
					d[i + wstep4] = mv.m256_f32[4];
					d[i + wstep5] = mv.m256_f32[5];
					d[i + wstep6] = mv.m256_f32[6];
					d[i + wstep7] = mv.m256_f32[7];
				}
...
#pragma omp parallel for
		for (int i = 0; i < src.cols; i++)
		{
			...
			for (int j = 0; j < r; j++)
			{
				...
				{
					int idx = border_s(j + k - r);
					v += (idx >= 0) ? gauss[k] * b[idx*wstep] : gauss[k] * constVal;
				}
				d[i] = v;
			}
			for (int j = r; j < src.rows - r; j++)
...
void GaussianFilterSeparableFIR::filterVHILineB(Mat& src, Mat& dest, const int r, const float sigma, int border, int opt, bool useAllocBuffer)
{
	...
	const int vmax = src.rows - 1;
	const int hmax = src.cols - 1;
	parallelCopyMakeBorder(src, buffer, 0, 0, 0, 8, border);
	if (opt == VECTOR_WITHOUT)
	{
		const int wstep = src.cols;
		const float* s = src.ptr<float>(0);
...
void GaussianFilterSeparableFIR::filterVHIImageBVBH(Mat& src, Mat& dest, const int r, const float sigma, int border, int opt, bool useAllocBuffer)
{
	...
	if (useParallelBorder)	parallelCopyMakeBorder(src, bufferImageBorder, r, r, 0, 16, border);
				...
				for (int j = -stop; j < TILE_Y + sbottom; j++)
				{
					...
					for (int i = TILE_X - RIGHT; i < TILE_X; i++)
					{
						...
						for (int k = 0; k < ksize; k++)
						{
							...
							v += (idx >= 0) ? gauss[k] * s[idx - tilex] : gauss[k] * constVal;
						}
						d[i] = v;
					}
				}
...
void GaussianFilterSeparableFIR::filterHVTileImageBH(Mat& src, Mat& dest, const int r, const float sigma, int border, int opt, bool useAllocBuffer)
{
	...
	if (!useAllocBuffer)bufferImageBorder.release();
	const int R = get_simd_ceil(r, 8);
	if (useParallelBorder)	parallelCopyMakeBorder(src, bufferImageBorder, 0, 0, R, R, border);
			...
			for (int t = 0; t < numTilesPerThread; t++)
			{
				...
				for (int j = -stop; j < TILE_Y + sbottom; j++)
				{
					...
					for (int i = 0; i < LEFT; i += 8)
					{
						...
						d[tstep0] = mv.m256_f32[0];
						d[tstep1] = mv.m256_f32[1];
						d[tstep2] = mv.m256_f32[2];
						d[tstep3] = mv.m256_f32[3];
						d[tstep4] = mv.m256_f32[4];
						d[tstep5] = mv.m256_f32[5];
						d[tstep6] = mv.m256_f32[6];
						d[tstep7] = mv.m256_f32[7];
						d += tstep8;
					}
			...
			for (int t = 0; t < numTilesPerThread; t++)
			{
				...
				for (int i = 0; i < left; i++)
				{
					...
					for (int j = 0; j < TILE_Y; j += 8)
					{
						...
						dii[wstep0] = mv.m256_f32[0];
						dii[wstep1] = mv.m256_f32[1];
						dii[wstep2] = mv.m256_f32[2];
						dii[wstep3] = mv.m256_f32[3];
						dii[wstep4] = mv.m256_f32[4];
						dii[wstep5] = mv.m256_f32[5];
						dii[wstep6] = mv.m256_f32[6];
						dii[wstep7] = mv.m256_f32[7];
						dii += dest.cols * 8;
					}
				}
...
void GaussianFilterSeparableFIR::filterHVBorder(Mat& src, Mat& dest, const int r, const float sigma, int border, int opt, bool useAllocBuffer)
{
	...
	if (useParallelBorder)	parallelCopyMakeBorder(src, bufferImageBorder, r, r, r, r + rem, border);
...
void GaussianFilterSeparableFIR::filterHVNonRasterBorder(Mat& src, Mat& dest, const int r, const float sigma, int border, int opt, bool useAllocBuffer)
{
	...
	if (useParallelBorder)	parallelCopyMakeBorder(src, bufferImageBorder, r, r + rem, r, r, border);
...
#pragma omp parallel for
		for (int i = 0; i < bufferImageBorder.cols; i++)
		{
			...
			for (int j = 0; j < src.rows; j += 8)
			{
				...
				s[wstep0] = mv.m256_f32[0];
				s[wstep1] = mv.m256_f32[1];
				s[wstep2] = mv.m256_f32[2];
				s[wstep3] = mv.m256_f32[3];
				s[wstep4] = mv.m256_f32[4];
				s[wstep5] = mv.m256_f32[5];
				s[wstep6] = mv.m256_f32[6];
				s[wstep7] = mv.m256_f32[7];
				s += wstep8;
			}
		}
		// h filter
		...
		for (int n = 0; n < max_core; n++)
		{
			...
			for (int i = start; i < end; i++)
			{
				...
				for (int j = 0; j < src.rows; j += 8)
				{
					...
					d[i + wstep20] = mv.m256_f32[0];
					d[i + wstep21] = mv.m256_f32[1];
					d[i + wstep22] = mv.m256_f32[2];
					d[i + wstep23] = mv.m256_f32[3];
					d[i + wstep24] = mv.m256_f32[4];
					d[i + wstep25] = mv.m256_f32[5];
					d[i + wstep26] = mv.m256_f32[6];
					d[i + wstep27] = mv.m256_f32[7];
				}
			}
		}
	}
			...
			for (int tilex = 0; tilex < src.cols; tilex += TILE_X)
			{
				...
				for (int j = 0; j < TILE_Y; j++)
				{
					for (int i = 0; i < TILE_X; i++)
					{
						...
						for (int k = 0; k < ksize; k++)
						{
							v += gauss[k] * blurx[j*(TILE_X + 2 * r) + i + k];
						}
						d[i] = v;
					}
					d += dest.cols;
			...
			for (int t = 0; t < numTilesPerThread; t++)
			{
				...
				for (int i = 0; i < TILE_X; i++)
				{
					...
					for (int j = 0; j < TILE_Y; j += 8)
					{
						...
						dii[dstep0] = mv.m256_f32[0];
						dii[dstep1] = mv.m256_f32[1];
						dii[dstep2] = mv.m256_f32[2];
						dii[dstep3] = mv.m256_f32[3];
						dii[dstep4] = mv.m256_f32[4];
						dii[dstep5] = mv.m256_f32[5];
						dii[dstep6] = mv.m256_f32[6];
						dii[dstep7] = mv.m256_f32[7];
						dii += dest.cols * 8;
					}
				}
			}
	...
	else if (opt == VECTOR_AVX)
	{
		...
		for (int j = 0; j < srcBorder.cols; j++)
		{
			for (int i = 0; i < tileSize.height; i += 8)
			{
				...
				s[(i + 0) * wstep] = mv.m256_f32[0];
				s[(i + 1) * wstep] = mv.m256_f32[1];
				s[(i + 2) * wstep] = mv.m256_f32[2];
				s[(i + 3) * wstep] = mv.m256_f32[3];
				s[(i + 4) * wstep] = mv.m256_f32[4];
				s[(i + 5) * wstep] = mv.m256_f32[5];
				s[(i + 6) * wstep] = mv.m256_f32[6];
				s[(i + 7) * wstep] = mv.m256_f32[7];
			}
			s++;
		}
		// h filter
	...
	else if (opt == VECTOR_AVX)
	{
		...
		for (int i = 0; i < tileSize.width; i++)
		{
			...
			for (int j = 0; j < tileSize.height; j += 8)
			{
				...
				d[i + dstep0] = mv.m256_f32[0];
				d[i + dstep1] = mv.m256_f32[1];
				d[i + dstep2] = mv.m256_f32[2];
				d[i + dstep3] = mv.m256_f32[3];
				d[i + dstep4] = mv.m256_f32[4];
				d[i + dstep5] = mv.m256_f32[5];
				d[i + dstep6] = mv.m256_f32[6];
				d[i + dstep7] = mv.m256_f32[7];
				d += dest.cols * 8;
			}
		}
